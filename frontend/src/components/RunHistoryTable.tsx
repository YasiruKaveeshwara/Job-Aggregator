  ...
  return (
    ...
    >
      ...
      >
        ...
        <tbody>
          {runs.map((run, i) => {
            const totalNew = Object.values(run.site_results).reduce(
              (s, r) => s + r.new,
              0
            );
            const totalFound = Object.values(run.site_results).reduce(
              (s, r) => s + r.found,
              0
            );
            const hasError = Object.values(run.site_results).some((r) => r.error);
            ...
            return (
              <tr
                ...
                style={{
                  borderBottom:
                    i < runs.length - 1 ? "1px solid var(--border-subtle)" : "none",
                }}
              >
                {/* ID */}
