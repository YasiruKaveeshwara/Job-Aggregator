...
import { startScrape, getScrapeStatus } from "@/lib/api";
...
export default function ScrapeControlPanel({ sources }: Props) {
  ...
  const totalNew = currentRun
    ? Object.values(currentRun.site_results).reduce((s, r) => s + r.new, 0)
    : 0;
    ...
    <div className="card" style={{ padding: 20 }}>
      ...
      {runState === "idle" && (
        <p style={{ fontSize: 12, color: "var(--text-muted)" }}>
          Click a button above to start fetching jobs.
        </p>
      )}
    </div>
  );
    ...
    >
      ...
      <span
        style={{
          width: 8, height: 8,
          borderRadius: "50%",
          background: done ? (result?.error ? "var(--red)" : "var(--green)") : color,
          flexShrink: 0,
          opacity: done ? 1 : 0.5,
        }}
      />
