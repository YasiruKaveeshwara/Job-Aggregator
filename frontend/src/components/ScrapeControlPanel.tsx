...
export default function ScrapeControlPanel({ sources, onSourcesChange, onRunFinished }: Props) {
	...
	const elapsedSec =
		currentRun?.status === "RUNNING" && startedAtMs ? Math.max(0, Math.floor((now - startedAtMs) / 1000))
		: currentRun?.duration_seconds != null ? Math.round(currentRun.duration_seconds)
		: startedAtMs && finishedAtMs ? Math.max(0, Math.floor((finishedAtMs - startedAtMs) / 1000))
		: 0;
			...
			{runState === "running" && (
				<div
					...
					<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
						...
						<span style={{ fontSize: "12px", fontWeight: "700", color: "var(--accent)" }}>{progressPercent}%</span>
					</div>
					...
					<div
						...
						<div
							style={{
								...
								height: "100%",
								borderRadius: "9999px",
								background: cancelling ? "var(--red)" : "linear-gradient(90deg, #6366f1 0%, #a855f7 50%, #10b981 100%)",
									...
									<span
										...
										style={{
											...
											color:
												...
												: "var(--text-muted)",
											border: `1px solid ${
												isDone ? "var(--green-border)"
												: isCurrent ? "var(--accent-border)"
												: "var(--border)"
											}`,
											display: "inline-flex",
											alignItems: "center",
		...
		<div
			...
			<div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
				<span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>
					{source.name}
					{isFetching && (
						<span style={{ marginLeft: 8, fontSize: 11, color: "var(--accent)", fontWeight: 600 }}>fetching…</span>
					)}
				</span>
				<span style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
					{source.last_scraped_at ?
						`Last fetched ${new Date(source.last_scraped_at).toLocaleDateString()} (${relativeTime})`
					:	"Never fetched"}
				</span>
			</div>
